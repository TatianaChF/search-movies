    ...
    const sortedBookmarks = (sortName) => {
        switch(sortName) {
            case "году выпуска (по возрастанию)":
                filteredBookmarks.value.sort((a, b) => a.year - b.year);
                break;
            case "году выпуска (по убыванию)":
                filteredBookmarks.value.sort((a, b) => b.year - a.year);
                break;
            case "рейтингу (по возрастанию)":
                filteredBookmarks.value.sort((a, b) => a.rating.kp - b.rating.kp);
                break;
            case "рейтингу (по убыванию)":
                filteredBookmarks.value.sort((a, b) => b.rating.kp - a.rating.kp);
                break;
            case "длительности (по возрастанию)":
                filteredBookmarks.value.sort((a, b) => a.movieLength - b.movieLength);
                break;
            case "длительности (по убыванию)":
                filteredBookmarks.value.sort((a, b) => b.movieLength - a.movieLength);
                break;
            default:
